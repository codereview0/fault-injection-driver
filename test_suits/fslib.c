...
int vp_stat(const char *file_name, struct stat *buf)
{
	int ret;
	...
	if ((ret = stat(file_name, buf)) < 0) {
		...
		sprintf(err_msg, "Error: Unable to stat file (name = %s statbuf = %x)", 
		file_name, (int)buf);
		vp_log_error(err_msg, err_code, EXIT_ON_ERROR);
	}
...
int read_file(int argc, char *argv[])
{
	...
	if (total_blocks > 0) {
		...
		for (i = 0; i < total_blocks; i ++) {
			...
			fprintf(stderr, "starting to read block %d\n", atoi(argv[2+i]));
			vp_read(fd, buffer, MY_BLOCK_SIZE);
			verify_block(buffer, atoi(argv[2+i]));
			fprintf(stderr, "block %d is read\n", atoi(argv[2+i]));
		}
	}
	else {
		/*read the whole file*/
		...
		for (i = 0; i < buf.st_size/MY_BLOCK_SIZE; i ++) {
			if (vp_read(fd, buffer, MY_BLOCK_SIZE) < 0) {
				vp_close(fd);
				return -1;
			}
...
/* 
 * write_file_async parameters: <file name> [<block#>, <block#>, ...]
 *
...
int write_file_async(int argc, char *argv[])
{
	...
	if (logging) {
		log_start_end_msgs(1, "write_file_async");
	}
	...
	if (logging) {
		log_start_end_msgs(0, "write_file_async");
	}
...
int stat_file(int argc, char *argv[])
{
	...
	sprintf(logmsg, "stat_file: %s", fname);
	printf("stat_file: %s", fname);
